...
sleep 1
cp -v /srv/dicom/gnumed_lh/index* /tmp/orthanc-backup/
invoke-rc.d orthanc start


TS=`date +%Y-%m-%d-%H-%M-%S`
cd /tmp
tar --strip-components=1 -cvJf ${BACKUP_DIR}/backup-orthanc-`hostname`-${TS}.tgz orthanc-backup/
cd -
